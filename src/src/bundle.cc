...
int bundle::build(vector<block> &blocks)
{
...
int bundle::process_paired_hits()
{
	imap.clear();
	for(int i = 0; i < hits.size(); i++)
	{
...
int bundle::build_blocks(vector<block> &blocks)
{
	set<PPI> s;
	...
	{
		//printf("try region %d\n", i);
		region r(chrm, v[i].first, v[i + 1].first, v[i].second, v[i + 1].second, &imap);
		block b;
		r.build_block(b);
